...
- フルカラー100%結果から各ステップの推測サイズを計算
- 推測サイズ一覧をコンソールに表示（デバッグ用）

### V121.22: 圧縮効率ベースの動的係数 + フルカラー70%ルール ✅ 実装完了

#### 圧縮効率ベースの動的係数
固定係数（256色=0.35等）では画像・エフェクトによって精度が大きく変動する問題を解決。

**方式:**
1. フルカラー100%で生成して圧縮効率を算出
   ```
   compressionRatio = fullColorSize / rawSize
   rawSize = width × height × 4 × frameCount
   ```
2. 圧縮効率から色数別係数を動的に計算
   ```typescript
   color256Factor = 0.15 + (compressionRatio * 0.8)
   color128Factor = 0.10 + (compressionRatio * 0.6)
   color64Factor = 0.08 + (compressionRatio * 0.5)
   ```

#### フルカラー70%ルール
「フルカラーでも作れる画像が256色になるのは避けたい」という要望に対応。

**ルール:**
- FC70%（面積49%）で制限内に収まる → フルカラー85%→70%を優先
- FC70%でも超過 → 256色100%から開始

**判定ロジック:**
```typescript
const fc70Size = fullColorSize * 0.70 * 0.70
...
#### 退場エフェクト修正
- `cardFlipOut`: effectOptionで回転数を設定（effectDirectionではなく）
- `pageFlipOut`: effectOptionで方向を設定（effectDirectionではなく）
- `generateFramesOnly`に全15種類の退場エフェクトを追加

#### 検証結果（5MB制限、3014x3727px画像）

| エフェクト | 推測 | 実際 | 精度 | 結果 |
|-----------|------|------|------|------|
| fadeIn | 3.52MB | 2.24MB | 64% | 256色100% ✓ |
| slideIn | 4.39MB | 4.91MB | 112% | FC70% ✓ |
| wipeIn | 4.04MB | 4.85MB | 120% | FC70% ✓ |
| zoomUp | 2.81MB | 3.25MB | 116% | 256色100% ✓ |
| doorClose | 2.94MB | 3.43MB | 117% | 256色100% ✓ |
| tvStaticIn | 4.31MB | 8.82MB | 205% | リトライ→128色60% |

**成功率**: 5/6（83%）一発成功、リトライ含めると100%成功

#### 既知の課題
1. **fadeIn過大推測**: 推測の37-64%（デルタ圧縮が非常に効く）
   - 品質を無駄に下げている可能性
2. **tvStaticIn過小推測**: 推測の134-205%（ノイズで圧縮効かない）
   - リトライで対応可能だが効率悪い

---

## 現在のステータス（2026年1月2日時点）

### 実装完了
- [x] 圧縮効率ベースの動的係数計算
- [x] フルカラー70%ルール
- [x] ループなし設定（setApngLoopCount）
- [x] 退場エフェクト15種類対応
- [x] Workerによる非同期エンコード
- [x] V121.23: エフェクト別デルタ圧縮補正係数

### 動作確認済み
- [x] ループなし設定: acTLチャンク修正が正常動作
- [x] プログレスバー: エンコード中100%表示、歯車回転継続
- [x] フルカラー70%ルール: slideIn/wipeInでFC70%選択成功

### 残課題
- [x] fadeInの過大推測改善 → V121.23で対応（補正係数0.5）
- [x] tvStaticInの推測精度改善 → V121.23で対応（補正係数2.0）

---

## V121.23 エフェクト別デルタ圧縮補正係数（2026年1月2日）

### 問題
- **fadeIn**: 過大推測（実際の37-64%）→ 品質を無駄に下げていた
- **tvStaticIn**: 過小推測（実際の134-205%）→ リトライが必要だった

### 解決策
エフェクト別の補正係数を導入し、`estimateSize`関数内で適用：

```typescript
const EFFECT_DELTA_FACTORS: Record<string, number> = {
    // 超高効率（推測を下げる）
    fadeIn: 0.5, fadeOut: 0.5,
    wipeIn: 0.6, wipeOut: 0.6,
    tileIn: 0.6, tileOut: 0.6,
    blindIn: 0.6, blindOut: 0.6,
    irisIn: 0.7, irisOut: 0.7,
    // 低効率（推測を上げる）
    tvStaticIn: 2.0, tvStaticOut: 2.0,
    glitchIn: 1.5, glitchOut: 1.5,
    zoomUp: 1.3, zoomUpOut: 1.3,
    doorClose: 1.3, doorOpen: 1.3,
}
```

### 期待される効果
- fadeIn: 推測精度 37-64% → 約80%（無駄な品質低下を防止）
- tvStaticIn: リトライ頻度低下（推測精度 134-205% → 約100%）
